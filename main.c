...
#define Z_TOLERANCE 0 // Tolerance for zx coordinate comparison

int x_draw = 320; // Start in the center of the window
int y_draw = 450; // Start in the center of the window
float thickness = 1; // Line thickness (Wireframe Mode Toggle)
...
void draw_line(int x1, int y1, int x2, int y2, float z1, float z2, SDL_Renderer *ren) {
    ...
    int dy = -abs(y2 - y1); // Difference in y coordinates (negative for SDL's regular coordinate system)
    ...
    float n = (abs(x2 - x1) > abs(y2 - y1)) ? abs(x2 - x1) : abs(y2 - y1); // Number of steps to take
    for (float i = 0; i <= n; i+=thickness) {
        float z = (z1 + (z2 - z1) * i / n); // Interpolated z value
        if (n == 0) z = z1; // If the line is a point, use z1
        int x = x1 + i * sx; // Interpolated x value
        int y = y1 + i * sy; // Interpolated y value
        int count = 0;
        if (x1 >= 0 && x1 < WINDOW_WIDTH && y1 >= 0 && y1 < WINDOW_HEIGHT) {
            //Only draw the pixel if its z value is greater than the current max z value for that x,y coordinate
            if (z > max_x[x1].y_coord[y1]) {
                ...
                max_x[x1].y_coord[y1] = z; // Update the max z value for this x,y coordinate
            }
        }
        else {
            continue;
